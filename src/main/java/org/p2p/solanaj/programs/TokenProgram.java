...
public class TokenProgram extends Program {
    ...
    private static final int INITIALIZE_MINT_METHOD_ID = 1;
    ...
    private static final int MINT_TO_CHECKED_METHOD_ID = 14;

    public static TransactionInstruction initializeMint(PublicKey mintAccount, byte decimals, PublicKey owner) {
        final List<AccountMeta> keys = new ArrayList<>();

        keys.add(new AccountMeta(mintAccount, true, true));
        keys.add(new AccountMeta(Sysvar.SYSVAR_RENT_ADDRESS, false, false));

        ByteBuffer result = ByteBuffer.allocate(35);
        result.order(ByteOrder.LITTLE_ENDIAN);

        result.put((byte) INITIALIZE_MINT_METHOD_ID);
        result.put(decimals);
        result.put(owner.toByteArray());
    ...
    public static TransactionInstruction mintToChecked(PublicKey mintAccount, PublicKey tokenAccount, long amount, byte decimals, PublicKey owner) {
        final List<AccountMeta> keys = new ArrayList<>();

        keys.add(new AccountMeta(mintAccount, false, true));
        ...
        result.put((byte) MINT_TO_CHECKED_METHOD_ID);
